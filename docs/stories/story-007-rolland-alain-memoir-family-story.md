...
- [x] Extend the local site build so the memoir appears alongside the family
      stories as a readable HTML story with an accessible main reading action
      and a reader-facing wrapper that does not force raw import/provenance
      affordances into the public page, while preserving the original PDF as
      repo-owned source material.
...
20260411-2340 — public-surface cleanup: removed the memoir wrapper's raw imported-HTML/original-PDF actions and the process-note copy in response to user direction that the public site should stay reader-facing while provenance moves to central or explicitly relevant archive surfaces later. Implementation touched `modules/build_family_site.py`, `scripts/deploy_static_site.py`, `input/doc-web-html/family-story-supplements.json`, touched-scope tests, deploy/docs truth surfaces, and refreshed `docs/omission-audit.json`; deterministic audiobook outputs were also regenerated to keep the repo green after the current renderer drifted from three checked-in chapters. Verification in this pass: `make build-audiobook-script FORCE=1`, `make build-family-site`, `make refresh-omission-audit`, `python -m pytest tests/test_build_family_site.py tests/test_deploy_static_site.py -q`, `python -m pytest tests/test_audiobook_script.py -q`, `python -m pytest tests/`, `python -m ruff check modules/ scripts/ tests/`, `make methodology-compile`, and `make methodology-check`. Result: the public memoir page now stays reader-facing, deploy-state collection excludes `_internal/` from the published payload, and Story 007 remains done. Next step: `/check-in-diff`.
