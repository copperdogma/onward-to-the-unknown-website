...
def test_build_family_site_emits_reader_facing_pages_and_internal_audit(tmp_path):
    ...
    assert ".nav-button.icon-only" in stylesheet
...
def test_build_family_site_surfaces_audiobook_page_and_entry_panel(tmp_path):
    ...
    assert 'class="audio-note audio-attribution"' in audiobook_html
    assert 'class="audio-attribution-icon"' in audiobook_html
    assert "These recordings were generated by AI using" in audiobook_html
    assert 'href="https://elevenlabs.io/"' in audiobook_html
    assert ">ElevenLabs</a>" in audiobook_html
    ...
    assert ".audio-attribution" in stylesheet
    assert ".audio-attribution-icon" in stylesheet
...
def test_build_family_site_surfaces_podcast_page_and_entry_panel(tmp_path):
    ...
    assert 'class="audio-note audio-attribution"' in podcast_html
    assert 'class="audio-attribution-icon"' in podcast_html
    ...
    assert "Use a Podcast App" not in podcast_html
    assert "This page stays the easiest place to listen here." not in podcast_html
    assert "Podcast RSS Feed" not in podcast_html
    assert 'rel="alternate" type="application/rss+xml"' in podcast_html
    assert 'title="Fixture Podcast podcast feed"' in podcast_html
    assert 'href="podcast/feed.xml"' in podcast_html
    assert 'class="nav-button secondary"' in podcast_html
    assert 'class="nav-button secondary icon-only"' not in podcast_html
    assert 'href="https://podcasts.apple.com/ca/podcast/fixture-podcast/id1234567890"' in podcast_html
    assert '<span class="nav-button-label">Apple Podcasts</span>' in podcast_html
    assert 'href="https://open.spotify.com/show/fixturepodcast"' in podcast_html
    assert '<span class="nav-button-label">Spotify</span>' in podcast_html
    ...
    assert channel.findtext("itunes:type", namespaces=namespaces) == "episodic"
    assert (
...
def test_build_family_site_omits_platform_buttons_when_manifest_urls_are_missing(tmp_path):
    ...
    assert 'href="podcast/feed.xml"' in podcast_html
    assert "Podcast RSS Feed" not in podcast_html
    assert "Apple Podcasts" not in podcast_html
    assert "Spotify" not in podcast_html
...
def test_repo_podcast_manifest_includes_feed_metadata():
    ...
    assert podcast_manifest["show_type"] == "serial"
    ...
    assert "NotebookLM" in podcast_manifest["description"]
    assert "AI-generated" in podcast_manifest["subtitle"]
    assert podcast_manifest["page_path"] in podcast_manifest["description"]
    assert podcast_manifest["spotify_url"] == "https://open.spotify.com/show/5PB4LuCnaxbdb41l6M4IWd"
...
def test_repo_podcast_manifest_publish_dates_support_platform_episode_order():
    repo_root = Path(__file__).resolve().parent.parent
    podcast_manifest = json.loads((repo_root / "podcast" / "manifest.json").read_text(encoding="utf-8"))

    full_book_date = podcast_manifest["full_book_episode"]["published_at"]
    chapter_dates = [episode["published_at"] for episode in podcast_manifest["episodes"]]
    listening_dates = [full_book_date, *chapter_dates]

    assert len(listening_dates) == len(set(listening_dates))
    assert chapter_dates == sorted(chapter_dates)
    assert full_book_date > chapter_dates[-1]
